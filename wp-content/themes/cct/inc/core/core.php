...
function cct_get_language_location()
{
$icon_en = cct_get_option('cct_flag_el') ? cct_get_option('cct_flag_el') : "";
$icon_ip = cct_get_option('cct_flag_jp') ? cct_get_option('cct_flag_jp') : "";


$flag_jp = isset($icon_ip["url"]) ? $icon_ip["url"] : "";
echo '<img src="' . $flag_jp . '">';

?>
<div class="header-top__right__listdropdown">
<ul class="add_class_active_lg">
<?php
$x = pll_languages_list(array('fields' => array()));
foreach ($x as $val) { ?>

<li><img src="<?php echo $val->flag_url ?>"> <a
href="<?php echo $val->home_url ?>"> <?php echo $val->name ?></a></li>
<?php
}
?>
</ul>
...
/**
 * Generate image class effect
 */
if (!function_exists('cct_generate_image_class_effect')) {
	function cct_generate_image_class_effect() {
		$image_effect = cct_get_option('image_effect', 'style-1');
