...
if (!function_exists('cct_get_breadcrumb_items')) {
	function cct_get_breadcrumb_items()
	{
		...
		/* Link to front page. */
		if (!is_front_page()) {
			$item[] = '<li><a href="' . esc_url(home_url('/')) . '" class="home">' . esc_html__('Home', 'cct') . '</a></li><i class="fas fa-chevron-right"></i>';
		}
		...
		if (function_exists('is_bbpress') && is_bbpress()) {
			...
			if ('page' !== $wp_query->post->post_type) {
				...
				if (function_exists('get_post_type_archive_link') && !empty($post_type_object->has_archive))
					$item[] = '<li><a href="' . esc_url(get_post_type_archive_link($post_type)) . '" title="' . esc_attr($post_type_object->labels->name) . '">' . $post_type_object->labels->name . '</a></li><i class="fas fa-chevron-right"></i>';
...
if (!function_exists('cct_filter_breadcrumb_items')) {
	function cct_filter_breadcrumb_items()
	{
		...
		if (get_option('page_on_front') != $shop_page_id) {
			...
			if (!is_shop()) {
				$item[] = '<li><a href="' . esc_url(get_permalink($shop_page_id)) . '">' . $shop_name . '</a></li><i class="fas fa-chevron-right"></i>';
			} else {
				$item['last'] = $shop_name;
			}
		}
		...
		if (!empty($current_term)) {
			...
			if (is_tax('product_cat') || is_tax('product_tag')) {
				...
				$item[] = '<li><a href="' . esc_url(get_term_link($current_term->term_id, $current_term->taxonomy)) . '">' . $current_term->name . '</a></li><i class="fas fa-chevron-right"></i>';
			}
		}
...
if (!function_exists('cct_breadcrumb_get_parents')) {
	function cct_breadcrumb_get_parents($post_id = '', $separator = '/')
	{
		...
		while ($post_id) {
			...
			$parents[] = '<li><a href="' . esc_url(get_permalink($post_id)) . '" title="' . esc_attr(get_the_title($post_id)) . '">' . get_the_title($post_id) . '</a></li><i class="fas fa-chevron-right"></i>';
...
if (!function_exists('cct_breadcrumb_get_term_parents')) {
	function cct_breadcrumb_get_term_parents($parent_id = '', $taxonomy = '', $separator = '/')
	{
		...
		while ($parent_id) {
			...
			$parents[] = '<li><a href="' . esc_url(get_term_link($parent, $taxonomy)) . '" title="' . esc_attr($parent->name) . '">' . $parent->name . '</a></li><i class="fas fa-chevron-right"></i>';
