...
if ( ! function_exists( 'cct_logo' ) ) {
	function cct_logo() {
		...
		$html        = array();
		$logo        = cct_get_option( 'cct_logo' ) ? cct_get_option( 'cct_logo' ) : array( 'url' => '' );
		$logoMobile  = cct_get_option( 'cct_logo_mobile' ) ? cct_get_option( 'cct_logo_mobile' ) : array( 'url' => '' );
		$alt         = esc_attr( get_bloginfo( 'name' ) );


		$html[] = '<div id="cct-site-logo" class="logo-main__left__logo">';

		if ( $logo['url'] ) {
			$html[] = '<a href="' . esc_url( $home_url ) . '">';
			$html[] = $logo ? '<img class="cct-logo d-none d-md-block" src="' . esc_url( $logo['url'] ) . '" alt="' . $alt . '"/>' : '';
			$html[] = $logoMobile ? '<img class="cct-logo-mobile d-block d-md-none" src="' . esc_url( $logoMobile['url'] ) . '" alt="' . $alt . '"/>' : '';
			$html[] = '</a>';
		} else {
...
if ( ! function_exists( 'cct_header_main_menu' ) ) {
	function cct_header_main_menu() {
		...
		$html[] = '<nav id="dl-menu" class="cct-site-nav dl-menuwrapper d-flex justify-content-md-center justify-content-end">';
		...
		if ( has_nav_menu( 'primary' ) ) {
			wp_nav_menu( array(
				...
				'menu_class'     => 'menu-main-nav dl-menu',
				'container'      => '',
...
if ( ! function_exists( 'cct_header_icon_cart' ) ) {
	function cct_header_icon_cart( $type = 'icon', $icon = false, $image = false ) {
		...
		$html[] = '<div class="logo-main__right__cart">';
		$html[] = '<a class="cctt-btn-cart" href="' . esc_url( wc_get_cart_url() ) . '">';

		$html[] = '<div class="logo_cart">';
		$html[] = '<i class="fa fa-shopping-bag"></i>';
        $html[] = '</div>';

        $html[] = '<div class="number_cart">';
